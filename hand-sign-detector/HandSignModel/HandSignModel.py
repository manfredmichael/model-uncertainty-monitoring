...
class HandSignModel:
    ...
    def __postprocessing_onnx(self, output_onnx, threshold, iou_threshold):
        ...
        for i in range(box_array.shape[0]):
            ...
            for j in range(num_classes):
                ...
                if keep.size > 0:
                    print("class probabilites:", confs[i][argwhere][keep])
    ...
    def __calculate_uncertainty(self, output_onnx, threshold, iou_threshold):
        box_array = output_onnx[0]
        confs = output_onnx[1]
        num_classes = confs.shape[2]
        box_array = box_array[:, :, 0]
        ...
        for i in range(box_array.shape[0]):
            ...
            for j in range(num_classes):
                ...
                keep = self.__nmsbbox(ll_box_array, ll_max_conf, iou_threshold)
                if keep.size > 0:
                    top_conf_1 = np.partition(confs[i][argwhere][keep].flatten(), -1)[-1]
                    top_conf_2 = np.partition(confs[i][argwhere][keep].flatten(), -2)[-2]
                    uncertainties.append(1 - (top_conf_1 - top_conf_2))
                    # ll_box_array = ll_box_array[keep, :]
                    # ll_max_conf = ll_max_conf[keep]
                    # ll_max_id = ll_max_id[keep]
                    # for k in range(ll_box_array.shape[0]):
    ...
    def get_uncertainty(self, img, threshold=None, iou_threshold=None):
        if threshold is None:
            threshold = self.threshold
        if iou_threshold is None:
            iou_threshold = self.nms_threshold
        ...
        input_onnx = self.ort_session.get_inputs()[0].name
        output_onnx = self.ort_session.run(None, {input_onnx: img})
        return self.__calculate_uncertainty(output_onnx, threshold, iou_threshold)

    def get_raw_bbox(self, img):
        img = self.__preprocessing_img(img)
        input_onnx = self.ort_session.get_inputs()[0].name
        output_onnx = self.ort_session.run(None, {input_onnx: img})
        return output_onnx
